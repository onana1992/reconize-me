...
export function CreateIntegrationButton({ product, liveUnlocked }: { product: string; liveUnlocked: boolean }) {
  const t = useT();
  ...
  async function onSubmit(formData: FormData) {
    ...
    setPending(false);
    if (!result.ok) {
      if (result.code === "live_locked" || result.code === "insufficient_credit") {
    ...
    <>
      ...
      >
        <div className="rm-dialog-head">
          <h2 id="create-integration-title">
            {issuedKey
              ? t("console.keys.issuedTitle").replace(
                  "{env}",
                  t(
                    issuedKey.startsWith("ky_live_")
                      ? "console.integrations.modeLive"
                      : "console.integrations.modeTest",
                  ),
                )
              : t("console.integrations.createTitle")}
          </h2>
        ...
        {issuedKey && createdId ? (
          ...
          <form ref={formRef} action={onSubmit} className="rm-form">
            ...
            {liveUnlocked ? null : (
              <p className="rm-hint">
                {t("console.integrations.liveLocked")}{" "}
